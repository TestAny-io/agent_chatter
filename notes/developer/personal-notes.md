...
## 历史 Bug 修复
1. Claude Code 无响应：原因 stdin 未关闭；修复为 sendAndReceive 后 `stdin.end()`.
2. 第二次消息报 “Process not running”：stdin 关闭导致进程退出但未清理；修复为显式调用 `stopAgent()`.
3. 等待人类输入无提示：`waitingForRoleId` 未设置；修复为启用自动轮询设置下一个角色。

## 待补充
- 个人实验、临时脚本、环境差异等记录请追加在此目录。
...
2. **Bug: 第二次消息失败 "Process not running"**
   ...
   - 修复：显式调用 `stopAgent()` 清理进程，下次自动重新启动
