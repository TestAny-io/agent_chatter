...
### 状态
- [x] 修复代码 - 已添加 `teamTask: this.session?.teamTask` 到 `buildPrompt` 调用
- [x] 添加单元测试验证
...
### 状态
- [x] 修复 JsonlMessageFormatter.ts - 已添加 `obj.role === 'assistant'` 条件
- [x] 修复 GeminiParser.ts - 根本原因是 parser 没有过滤 user 消息，已修复
- [x] 添加单元测试

### 影响
此 Bug 导致了 Bug 5（Context 递归污染），是最严重的根因之一。**已修复**。
...
### 修复
修复 Bug 4 后，此问题应自动解决。**已随 Bug 4 修复一起解决。**
...
与 Bug 3 是同一个问题，合并处理。**已随 Bug 3 修复一起解决。**

---

## Bug 7: 并行 NEXT 时后续 agent 缺少 [CONTEXT]，[MESSAGE] 是原始消息而非上一个 agent 的回复

### 现象
用户发 `[NEXT:max] [NEXT:sarah] [NEXT:carol]` 时：
- Max：收到 `[MESSAGE]` = 用户消息 ✅
- Sarah：收到 `[MESSAGE]` = 用户消息（应为 Max 的回复），无 `[CONTEXT]`
- Carol：收到 `[MESSAGE]` = 用户消息（应为 Sarah 的回复），无 `[CONTEXT]`

### 根因
`ConversationCoordinator.routeToNext()` 第 419-423 行：
```typescript
...
三个成员在**同一时刻**被入队，此时 `message.content` 都是用户的原始消息。
入队时预先计算了 content，而不是在处理时动态获取。

### 修复方案
队列中不存 content，处理时动态获取：
1. 从 `session.messages` 获取最新一条消息作为 `[MESSAGE]`
2. `[CONTEXT]` 是最新消息之前的历史

### 状态
- [x] 修改 routingQueue 结构 - 改为只存 `{ member }` 而非 `{ member, content }`
- [x] 修改 processRoutingQueue 逻辑 - 处理时动态从 `session.messages` 获取最新消息
- [x] 添加单元测试 - `routingQueue.integration.test.ts` 新增 `uses dynamic latest message for parallel NEXT routing` 测试

### 修复代码变更
`ConversationCoordinator.ts`:
1. 第 63 行：`routingQueue` 类型改为 `Array<{ member: Member }>`
2. 第 419-422 行：入队时只存 member
3. 第 441-446 行：处理时动态获取最新消息
