...
export async function checkConnectivity(
  ...
  logger.debug(`Starting connectivity check for agent: ${agentType}`);
  ...
  logger.debug(`Endpoint: ${endpoint?.host}:${endpoint?.port}, httpPath: ${httpPath}`);
  logger.debug(`Timeouts - DNS: ${dnsTimeout}ms, TCP: ${connectivityTimeout}ms, HTTP: ${httpTimeout}ms`);
  logger.debug(`skipHttpCheck: ${skipHttpCheck}`);

  // Unknown Agent type, skip check
  if (!endpoint) {
    logger.debug(`Unknown agent type "${agentType}", skipping check`);
  ...
  try {
    ...
    if (!skipHttpCheck && httpPath) {
      logger.debug(`[Layer 7] Starting HTTP check to https://${endpoint.host}${httpPath}...`);
      ...
      if (!httpResult.reachable) {
        logger.debug(`[Layer 7] HTTP check failed - returning error`);
        return {
          ...httpResult,
      ...
      logger.debug(`[Layer 7] Skipped (skipHttpCheck=${skipHttpCheck}, httpPath=${httpPath})`);
    }

    logger.debug(`Connectivity check completed successfully (${Date.now() - startTime}ms)`);
    return {
      reachable: true,
      latencyMs: Date.now() - startTime,
    };
  ...
  return new Promise((resolve) => {
    const req = https.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          logger.debug(`[HTTP] Response body: ${body.slice(0, 500)}`);
          const result = classifyHttpResponse(host, statusCode, body);
          logger.debug(`[HTTP] Classified result: ${JSON.stringify(result)}`);
          resolve(result);
        });
      }
    );
    ...
    req.on('timeout', () => {
      logger.debug(`[HTTP] Request timeout after ${timeoutMs}ms`);
      req.destroy();
      resolve({
        reachable: false,
