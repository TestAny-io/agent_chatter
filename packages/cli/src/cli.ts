...
import type { ToolStatus, ILogger } from '@testany/agent-chatter-core';
...
import { ConsoleLogger } from './outputs/ConsoleLogger.js';
 ...
 * Priority: CLI flag > environment variable > default (false)
 */
function isDebugEnabled(cliFlag?: boolean): boolean {
  if (cliFlag !== undefined) {
    return cliFlag;
  }
  const envVar = process.env.AGENT_CHATTER_DEBUG;
  return envVar === '1' || envVar === 'true';
}

/**
 * Create a logger instance based on debug setting
 */
function createLogger(debug: boolean): ILogger {
...
program
    ...
    .option('--debug', 'Enable debug logging (also: AGENT_CHATTER_DEBUG=1)')
    .action(async () => {
        // 当没有子命令时，启动REPL模式
        const cliOptions = program.opts<{ registry?: string; debug?: boolean }>();
        const debug = isDebugEnabled(cliOptions.debug);
        startReplInk(cliOptions.registry, { debug });
    });
