 ...
 *
 * Core 层：依赖通过 options 注入，不直接创建执行实现
 */
...
import type { AgentDefinition, TeamMemberConfig } from '../models/CLIConfig.js';
import type { CoreTeamConfig } from '../models/CoreTeamConfig.js';
...
export interface InitializeServicesOptions {
  /** Logger for Core diagnostic messages (required) */
  logger: ILogger;
  /** Execution environment for spawning agent processes (required, injected by CLI) */
 ...
 *
 * Core 层返回：不包含执行实现（ProcessManager 等）
 */
export interface InitializeServicesResult {
  ...
  /** Verification results for each agent type (CLI layer formats and displays) */
 ...
 *
 * Core 层入口：executionEnv 和 adapterFactory 由 CLI 层注入
 *
 * @param config - CoreTeamConfig (不包含 UI 偏好字段)
 * @param options - 初始化选项
 */
export async function initializeServices(
  config: CoreTeamConfig,
  options: InitializeServicesOptions
): Promise<InitializeServicesResult> {
  const logger: ILogger = options.logger ?? new SilentLogger();
  const { executionEnv, adapterFactory } = options;

  // Validate required dependencies
  if (!executionEnv) {
    throw new Error('initializeServices requires executionEnv to be provided in options');
  }
  if (!adapterFactory) {
    throw new Error('initializeServices requires adapterFactory to be provided in options');
  }
  ...
  for (const [index, member] of config.team.members.entries()) {
    ...
    if (member.type === 'ai') {
      ...
      if (isFirstVerification) {
        ...
        // CLI layer will format detailed output from verificationResults
        // Use debug level here to avoid duplicate output
        logger.debug(`Agent ${member.agentType} verification: ${verification.status}`);
      } else {
        logger.debug(`Agent ${member.agentType} using cached verification`);
      }
  ...
  const coordinator = new ConversationCoordinator(
    ...
    messageRouter,
    {
      ...
      onMessage: options?.onMessage ?? ((message: ConversationMessage) => {
        ...
        logger.info(`[${timestamp}] ${speaker.displayName}: ${message.content}`);
      }),
  ...
  return {
    ...
    eventEmitter,
    contextCollector,
    verificationResults: verificationCache
  };
}
